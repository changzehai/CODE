...
static void ut4b0_gga_msg_parse(u8_t *pdata)
{
	...
	strcpy((char *)gsatellite_info.location.alt, (const char *)p);	
            ...
            MTFS30_DEBUG("UTC时间： %s\n纬度: %s\n纬度方向: %c\n经度： %s\n经度方向： %c\n使用中的卫星数： %c\n海拔： %s\n",
                         gtime.hms, gsatellite_info.location.lat, gsatellite_info.location.latdir,
                         gsatellite_info.location.lon, gsatellite_info.location.londir, 
                         gsatellite_info.use_sats, gsatellite_info.location.alt);          
#endif	
...
#if MTFS30_DEBUG_EN
            MTFS30_DEBUG("信息总数： %d\n信息数： %d\n可见的卫星总数： %d\n",
                         gsv_msg_total, gsv_msg, gsv_amount_stas);
	...
	else
	{
		sprintf((char *)gleap_forecast.hms, "%s", "075958"); /* 北京时间负闰秒发生时间为: 23:59:58 */
            ...
            MTFS30_DEBUG("新的闰秒发生年： %s\n新的闰秒发生年： %s\n新的闰秒发生月： %s\n新的闰秒发生日： %s\n新的闰秒值： %d\n正/负闰秒： %d\n闰秒发生时分秒： %s\n",
                         gleap_forecast.year, gleap_forecast.year, gleap_forecast.mon,
                         gleap_forecast.day, gleap_forecast.next_leap, gleap_forecast.sign, gleap_forecast.hms);
