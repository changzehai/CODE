...
s32_t snmpVCompare(const u8_t *op1, s16_t len1,
    const u8_t *op2, s16_t len2)
{
    s16_t i1, len;
    s32_t cmp;
    u32_t  r1,r2;
    ...
    /* Encode values into a single signed 32 bit value */
    cmp <<= 16;
    if (len - i1)
        cmp *= len - i1;
    cmp += len1;
    ...
    s16_t tabix,const MIBVAR *mvpold)
{
    ...
    for (i1 = mtp->nix; i1; i1--)
    {
        ...
        else
        {
            ...
            {
	        /* SCALAR variables are not indexed */
	        if ((mvp->opt & SCALAR) == 0)
	            vptr += mtp->len * tabix;
...
s32_t snmpFindBulkIndex(s16_t *tabixp, const MIBTAB *mtp,
    const MIB *mibp, const MIBVAR *mvp,
    const u8_t *reqixname, u8_t reqixlen, u8_t nflag)
{
    u8_t *cp;
	    ...
	    for (i1 = 0; ; i1++)
	    {
	        ...
	        */
	        sl1 = snmpVCompare(cp, (const u8_t *)&ixname[MAXKLEN] - cp,
	       	...
	       	if (sl1 == 0)
	       	{
	        	i3 = mibp->index(mvp - mibp->mvp, i1 + *tabixp);
	        	if (i3 > 0)
	        	{
		        	sl2 = 1;
		        	i1 = i1 + *tabixp;
