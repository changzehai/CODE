...
#ifdef DEBUG_MTFS30_1
            MTFS30_DEBUG("[%s:%d] 卫星消息: %s\n", __FUNCTION__, __LINE__, gusart1_rx_buf);
#endif         
...
//                 (OS_ERR*   ) &err);       
    ...
    /* 创建串口1消息队列 */
    OSQCreate((OS_Q*      )&gusart1_msg,         /* 串口1消息队列 */
              (CPU_CHAR*  )"USART1 MSG",
              (OS_MSG_QTY )1,
              (OS_ERR*    )&err);  
    ...
    OSMemCreate((OS_MEM     *)&gusart1_mem,
               (CPU_CHAR    *)"USART1 MEM",
               (void        *)pusart1_mem_addr,   /* 内存分区起始地址       */
               (OS_MEM_QTY   )USART1_MSGQEUE_SIZE,/* 内存分区里的内存块数量 */
               (OS_MEM_SIZE  )USART1_MSGQEUE_LEN,    /*每个内存块的大小(字节)  */
               (OS_ERR      *)&err);  
